...
var sendProductDeletedRequest = (idPrd,deleteType) => {
...
$("button.btn-delete-one").on("click",function(e){
	...
	sendProductDeletedRequest(idPrd,'sync');
});
...
$("button.btn-delete-all").on("click",function(e){
	...
	$("input.deleted-product-checkbox:checked").each(function(){
		...
		sendProductDeletedRequest(this.getAttribute('value'),'async');
	});
});
...
var updatePicModule = (function($){
	...
	function _updatePic(){
		...
		if (ctrlUpdatePicsProcess == false){
			...
			console.log('After ========<<<<<<<<<<<');
			console.log(updatePicsSkuArr);
			console.log("MMMMAAAAAA---->");

			if (updatePicsSkuArr.length == 0){
				updatePicsSkuArr.push(1);
				$(".updatepics__console").html("No hay imágenes vacias.");
