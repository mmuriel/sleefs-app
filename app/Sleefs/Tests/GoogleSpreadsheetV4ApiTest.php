...
class GoogleSpreadsheetV4ApiTest extends TestCase {
    ...
    public function testGetCtrlWorkSheet(){
    	...
    	$this->assertMatchesRegularExpression(3,$index1);
    	$this->assertMatchesRegularExpression(false,$index2);
    ...
    public function testLockSpreadsheetFile(){
    	...
    	$this->assertMatchesRegularExpression(true,$resLock1);
    	$this->assertMatchesRegularExpression(false,$resLock2);

    	//Verifica que el contenido de la celda A1 sea = locked
		$worksheet = $worksheets[$index];
		$cellFeed = $worksheet->getCellFeed();
		$cell = $cellFeed->getCell(1,1);
		$this->assertMatchesRegularExpression('locked',$cell->getContent());		
