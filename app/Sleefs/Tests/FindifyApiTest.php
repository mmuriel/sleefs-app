...
class FindifyApiTest extends TestCase {
    ...
    public function testLoginToFindifyApi(){
    	...
    	$this->assertMatchesRegularExpression('admin@sleefs.com',$loginResult->notes->user->email);
    ...
    public function testLoginToFindifyApiError(){
    	...
    	$this->assertMatchesRegularExpression('Username or password not correct',$loginResult->notes->error->message);
    ...
    public function testGetAllCollections(){
    	...
    	$this->assertMatchesRegularExpression(56233,$collections[0]->id,"Se ha cambiado el ID de la colección 'NEW', hay que cambiar ese código");
    ...
    public function testSearchForNewCollection(){
    	...
    	$indexLastAddedItem--;
    	$this->assertMatchesRegularExpression('MMMMTESTING',$response->query->filters[0]->values[$indexLastAddedItem]->value);
    	//Elimina el ultimo registro
    	array_pop($response->query->filters[0]->values);
    	$response = $findifyApi->updateCollection($response,250,250);
    	$this->assertMatchesRegularExpression($indexLastAddedItem,count($response->query->filters[0]->values));
    }
